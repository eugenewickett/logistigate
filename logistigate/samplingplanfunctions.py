...
def sampling_plan_loss_fast(design, numtests, priordatadict, paramdict):
    """
    Estimates the sampling plan loss for a testing budget under a given data set and loss parameters, using the
    efficient estimation algorithm; uses matrix multiplication, which can be expensive to calculate.
...
# FOR GETTING MAX UTILITY FROM MATRIX OF UTILITIES ALONG NODES/PATHS
##################################
...
def get_opt_allocation(U):
    ...
    Each element of U should contain utilities corresponding to a number of tests at that test node or trace.
    """
    Udim = np.ndim(U)
...
# END GETTING MAX UTILITY FROM MATRIX OF UTILITIES ALONG NODES/PATHS
##################################
##################################
##################################