...
if __name__ == '__main__':
    unittest.main()
'''
t = util.generateRandDataDict(numImp=5, numOut=20, numSamples=50 * 20, dataType='Tracked', randSeed=2)
_ = util.GetVectorForms(t)


nOut, nImp = len(t['outletNames']), len(t['importerNames'])
# Set an initial beta
beta0 = np.ones(nImp + nOut)*-2
# Use a non-default prior
prior = prior_normal(mu=1,var=2)
# Grab the likelihood and gradient at beta0
L0 = methods.Tracked_LogPost(beta0, t['N'], t['Y'],t['diagSens'], t['diagSpec'],prior)
dL0 = methods.Tracked_LogPost_Grad(beta0, t['N'], t['Y'], t['diagSens'], t['diagSpec'],prior)
...
for k in range(nImp + nOut):
    beta1 = 1 * beta0[:]
    beta1[k] = beta1[k] + 10 ** (-5)
    L1 = methods.Tracked_LogPost(beta1, t['N'], t['Y'], t['diagSens'], t['diagSpec'],prior)
    print(np.abs((L1 - L0) * (10 ** (5)) - dL0[k]))
    if np.abs((L1 - L0) * (10 ** (5)) - dL0[k]) > 0.001:
        flag = True
...
# Grab the likelihood and gradient at beta0
dL0 = methods.Tracked_LogPost_Grad(beta0, t['N'], t['Y'], t['diagSens'], t['diagSpec'],prior)
ddL0 = methods.Tracked_LogPost_Hess(beta0, t['N'], t['Y'],t['diagSens'], t['diagSpec'],prior)
# Move in every direction and flag if the difference from the gradient is more than epsilon
flag = False
for k in range(nImp + nOut):
    beta1 = 1 * beta0[:]
    beta1[k] = beta1[k] + 10 ** (-5)
    dL1 = methods.Tracked_LogPost_Grad(beta1, t['N'], t['Y'],
                                      t['diagSens'], t['diagSpec'],prior)
    print(np.linalg.norm((dL1 - dL0) * (10 ** (5)) - ddL0[k]))
    if np.linalg.norm((dL1 - dL0) * (10 ** (5)) - ddL0[k]) > 0.001:
        flag = True
'''